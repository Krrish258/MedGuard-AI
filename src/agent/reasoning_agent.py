...
CONFIDENCE_THRESHOLD = 0.40   # below this → abstain recommended
...
class ReasoningAgent:
    ...
    def _load_treatment_kb(self):
        if not self._treatment_kb:
            path = KB_DIR / "treatment_guidelines.json"
            with open(path) as f:
                self._treatment_kb = json.load(f)
    ...
    def _tool_predict_diagnosis(
        ...
        """Run the ensemble classifier to get diagnosis + confidence."""
        self._load_model()
    ...
    def _tool_generate_treatment(
        ...
        lookup_key = diagnosis
        if guideline_match and guideline_match["similarity_score"] > 0.7:
            lookup_key = guideline_match["disease"]

        data = self._treatment_kb.get(lookup_key, self._treatment_kb.get(diagnosis, {}))

        if not data or data.get("first_line_drug") in (None, ""):
            ...
            drug_name   = treatment.split("+")[0].strip()
    ...
    def run(self, evidence: EvidenceBundle) -> AgentResult:
        ...
        diagnosis, confidence, differential = self._tool_predict_diagnosis(feature_vector)
